...
function Shell() {
  ...
  // The pushed goal detail, openable from both Goals and Today. It lives above the Goals
  // tab, so tapping a tab always lands on that tab's own screen rather than a leftover
  // detail view.
  ...
  function openGoalDetail(goalId: string | null) {
    setGoalDetailId(goalId)
    if (goalId) setTabState('goals')
  }
  ...
  return (
    <NavContext value={{ tab, setTab, goalDetailId, openGoalDetail }}>
      <div className="flex h-full flex-col">
        <div className="min-h-0 flex-1 overflow-hidden">
          ...
          {tab === 'goals' &&
            (goalDetailId ? (
              <GoalDetail goalId={goalDetailId} onBack={() => openGoalDetail(null)} />
            ) : (
              <Goals />
            ))}
          {tab === 'journal' && <Journal />}
