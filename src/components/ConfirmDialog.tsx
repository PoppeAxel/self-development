    ...
    <div className="fixed inset-0 z-[80] flex items-center justify-center bg-black/40 px-6" onClick={onCancel}>
      <div className="w-full max-w-xs rounded-3xl border border-line bg-surface p-5 shadow-card" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="mt-4 flex gap-2">
          <button onClick={onCancel} className="flex-1 rounded-[20px] bg-track px-4 py-2.5 font-medium text-ink-2">
            {cancelLabel}
          </button>
          <button onClick={onConfirm} className="flex-1 rounded-[20px] bg-cat-rose px-4 py-2.5 font-semibold text-white">
            {confirmLabel}
          </button>
        </div>
      </div>
